...
# 2. Define a potential (e.g., a harmonic oscillator)
# This function computes forces and optionally the potential energy
function harmonic_force(positions; return_potential=false)
    forces = [-p for p in positions]
    if return_potential
        potential_energy = 0.5 * sum(dot(p, p) for p in positions)
...
dt = 0.1
for _ in 1:10
    velocity_verlet!(sys, harmonic_force, dt)
end
sys.positions
```
