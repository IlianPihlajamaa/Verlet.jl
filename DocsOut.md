Good catch — that error comes from a mismatched code fence in `docs/src/index.md`. One of the `@example` blocks was closed with **four** backticks instead of three, which makes the next header look like it’s inside a string.

Here’s a corrected `docs/src/index.md` you can paste over the current file (only fence fixes and tiny polish):
...
Then rebuild the docs the same way you did before. If anything else pops up, paste the new error and I’ll patch it.
```
