...
"""
    velocity_verlet!(system::ParticleSystem{T}, forces::Function, dt::T)

Advance `system` by one timestep `dt` using the velocity Verlet integrator.
"""
function velocity_verlet!(system::ParticleSystem{T}, forces::Function, dt::T) where {T}
...
"""
    potential_energy(system::ParticleSystem{T}, forces::Function) -> T

Try to obtain total potential energy using `forces(...; return_potential=true)` convention.
"""
function potential_energy(system::ParticleSystem{T}, forces::Function) where {T}
    F_U_kw = try
